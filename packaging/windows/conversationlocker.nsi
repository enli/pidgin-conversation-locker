; Script based on pidgin-facebookchat plugin NSI which in turn
; was based on the Skype4Pidgin and Off-the-Record Messaging NSI files
...
!define PRODUCT_NAME "pidgin-conversation-locker"
!define PRODUCT_VERSION "1.2"
!define PRODUCT_PUBLISHER "Peeyoosh Sangolekar"
...
;	SetOutPath "$PidginDir\ca-certs"
	...
	copy:
		ClearErrors
		Delete "$PidginDir\plugins\conversationlocker.dll"
		IfErrors dllbusy
		SetOutPath "$PidginDir\plugins"
	    File "conversationlocker.dll"
		Goto after_copy
	dllbusy:
		MessageBox MB_RETRYCANCEL "Plugin file conversationlocker.dll is busy. Please close Pidgin (including tray icon) and try again" IDCANCEL cancel
		Goto copy
	cancel:
		Abort "Installation of pidgin-conversation-locker-plugin aborted"
	after_copy:
